from car.models import Byline, Datatype, Nation, Source, State, Story, Topic, Type, Application, Database
...
def state_detail(request, state):
    ...
    state_list = Source.objects.filter(state=s).order_by('name').annotate(stories=Count('story__id')).order_by('-stories')
